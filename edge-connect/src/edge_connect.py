...
class EdgeConnect():
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if self.config.CONF:
                    images, images_gray, edges, masks, confmap = self.cuda(*items)
                else:
                ...
                if model == 1:
                    # train
                    if self.config.OSGAN:
                        outputs, gen_loss, dis_loss, logs, loss_pack = self.edge_model.process(images_gray, edges, masks, scaler)
                    else:
                        outputs, gen_loss, dis_loss, logs, _ = self.edge_model.process(images_gray, edges, masks)
                    ...
                    # backward
                    if self.config.OSGAN:
                        self.edge_model.backward(gen_loss, dis_loss, loss_pack)
                    else:
    ...
    def eval(self):
        ...
        for items in val_loader:
            ...
            # inpaint model
                ...
                # eval
                outputs, gen_loss, dis_loss, logs, loss_pack = self.inpaint_model.process(images, edges, masks)
                outputs_merged = (outputs * masks) + (images * (1 - masks))
    ...
    def test(self):
        ...
        valpath = os.path.join(self.results_path,"val")
        edgepath = os.path.join(self.results_path,"edge")
        maskedpath = os.path.join(self.results_path,"masked")
        create_dir(valpath)
        for i in range(0,110,10):
            create_dir(os.path.join(self.results_path,str(int(i/10))))
            create_dir(os.path.join(valpath,str(int(i/10))))
            create_dir(os.path.join(edgepath,str(int(i/10))))
            create_dir(os.path.join(maskedpath,str(int(i/10))))
        ...
        for items in test_loader:
            ...
            # inpaint with edge model / joint model
                ...
                outedges = self.edge_model(images_gray, edges, masks).detach()
                outputs = self.inpaint_model(images, outedges, masks)
                outputs_merged = (outputs * masks) + (images * (1 - masks))
                #inputs_edge = (masks) + (edges * (1 - masks))
                #inputs_gray = (masks) + (images_gray * (1 - masks))

            output = self.postprocess(outputs_merged)[0]
            i = int( (torch.sum(masks) / 65536 * 10 ) )
            
            path = os.path.join(self.results_path, str(i), name)
            print(index, name)
            path2 = os.path.join(valpath, str(i), name)
            imsave(output, path)
            output2 = self.postprocess(images)[0]
            imsave(output2, path2)

            if self.debug or True:
                outedges = self.postprocess(outedges)[0]
                masked = self.postprocess(images * (1 - masks) + masks)[0]
                fname, fext = name.split('.')
                imsave(outedges, os.path.join(edgepath, str(i), name))
                imsave(masked, os.path.join(maskedpath, str(i), name))
