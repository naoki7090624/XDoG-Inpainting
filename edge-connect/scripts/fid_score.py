...
import warnings
warnings.simplefilter('ignore') # ignore user warning
...
def calculate_fid_given_paths(paths, batch_size, cuda, dims):
    ...
    for i in range(0,7):
        print("Mask size: ", i*10)
        p1 = os.path.join(paths[0],str(i))
        p2 = os.path.join(paths[1],str(i))
        print('calculate path1 statistics...')
        m1, s1 = _compute_statistics_of_path(p1, model, batch_size, dims, cuda)
        print('calculate path2 statistics...')
        m2, s2 = _compute_statistics_of_path(p2, model, batch_size, dims, cuda)
